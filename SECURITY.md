...
Après déploiement — scan dynamique de l'app live, et, sur un environnement de staging isolé, un pentest (OWASP ZAP, puis éventuellement un pentester IA).
...
AgentShield (issu d'ECC) — audite la configuration des agents (CLAUDE.md, settings.json, MCP, hooks) : secrets, permissions, injections : npx ecc-agentshield scan.
