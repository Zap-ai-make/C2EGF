# WORKFLOW.md — Du cahier des charges au produit livré
...
Tout ce pipeline s'exécute sous `AGENTS.md` et les trois contrats déjà en place : `DESIGN.md`, `SECURITY.md`, `ARCHITECTURE.md`. Ce fichier ne les répète pas, il les ordonne dans le temps.
...
1. **Après l'analyse du cahier des charges** — si quoi que ce soit est ambigu, il pose ses questions avant de découper. Il n'invente pas un besoin.
2. **Après la définition du périmètre MVP** — il présente le découpage et la ligne MVP / post-MVP, et attend validation avant de construire.
3. **À la livraison du MVP** — il s'arrête, démontre, et attend le feu vert avant toute feature post-MVP.
...
**Où vivent les specs :** un dossier `specs/` à la racine, un fichier par spec nommé `specs/S<n>-<slug>.md` (ex. `specs/S1-inscription-email.md`).

**Le fichier `specs/ROADMAP.md`** est la source de vérité de la progression (cf. §9). Un tableau suffit, une ligne par spec :

```
| ID | Spec                     | Dépend de | Périmètre | Statut   |
|----|--------------------------|-----------|-----------|----------|
| S1 | Inscription par e-mail   | aucune    | MVP       | terminée |
| S2 | Tableau de bord — liste  | S1        | MVP       | en cours |
| S3 | Export CSV               | S2        | post-MVP  | à faire  |
```

**Sortie de phase :** `specs/ROADMAP.md` listant toutes les specs, leurs dépendances et leur ordre.
...
1. **Plan court** — ce qui va être fait, les fichiers touchés, les points de sécurité éventuels. (Agent `planner` si ECC est installé — cf. `ECC.md`.)
2. **Implémentation minimale** — la version juste qui satisfait les critères d'acceptation, pas plus (échelle `ARCHITECTURE.md` §1). Réutiliser l'existant avant d'écrire du neuf.
3. **Vérification** — exécuter le code, lancer les tests, et pour l'UI regarder le rendu (capture). « Ça compile » n'est pas « ça marche » (`ARCHITECTURE.md` §4).
4. **Revue contre les trois contrats** : `SECURITY.md` (auth/données/entrées), `DESIGN.md` (UI/états/accessibilité), `ARCHITECTURE.md` (qualité/structure).
5. **Commit** — un commit propre par spec, message qui dit le pourquoi (`ARCHITECTURE.md` §11). Marquer la spec terminée dans la roadmap.
...
- **S'arrêter plutôt que deviner** — sur une ambiguïté du cahier des charges ou une opération sensible (données, paiement, migration), demander (`AGENTS.md` règle 6).
...
Les phases 3 et 4 sont répétitives par nature — « pour chaque spec : plan → implémentation → vérification → revue → commit ». C'est exactement une boucle. Si ECC est installé, l'agent `loop-operator` peut la piloter (`/loop-start`, `/loop-status`, `/quality-gate` — cf. `ECC.md`) ; sinon, l'agent applique la même discipline manuellement, une spec après l'autre.
